...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='TMKGroups' AND COLUMN_NAME='OrganizationID') ALTER TABLE TMKGroups ADD [OrganizationID] INT NOT NULL
...
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID('TMKGroups') AND name = 'IX_TMKGroups_OrganizationID') CREATE NONCLUSTERED INDEX [IX_TMKGroups_OrganizationID] ON [TMKGroups] (OrganizationID ASC)